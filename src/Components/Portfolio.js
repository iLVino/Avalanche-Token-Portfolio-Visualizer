...
const availableChains = ['avalanche-mainnet', 'defi-kingdoms-mainnet', 'avalanche-dexalot-mainnet', 'avalanche-shrapnel-mainnet', 'avalanche-step-network', 'avalanche-meld-mainnet']; // Add more chains as needed
...
function Portfolio() {
  ...
  const [selectedChain, setSelectedChain] = useState(availableChains[0]); // Default to the first chain
  ...
  */
  ...
  const handleButtonClick = () => {
    ...
    const historicPortfolioValueEndpoint = `https://api.covalenthq.com/v1/${selectedChain}/address/${walletAddress}/portfolio_v2/`

    fetch(historicPortfolioValueEndpoint, { method: 'GET', headers: { "Authorization": `Basic ${btoa(apiKey + ':')}` } })
      .then(res => res.json())
      .then(res => {
